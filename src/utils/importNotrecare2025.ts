...
function calculateAppointmentDate(diaNumero: string, mes: 'Novembro' | 'Dezembro'): Date | null {
  ...
  const month = mes === 'Novembro' ? 10 : 11; // 10 = novembro, 11 = dezembro
...
function extractDiagnosticos(diagnostico: string): {
  ...
  if (diagLower.includes('dmg') || diagLower.includes('diabetes gestacional')) {
    maternos.push('Diabetes Mellitus Gestacional');
  }
  if (diagLower.includes('hac') || diagLower.includes('hipertens') || diagLower.includes('pre-eclampsia') || diagLower.includes('eclampsia')) {
  ...
  if (diagLower.includes('bariatric')) {
    maternos.push('Pós-bariátrica');
  }
  if (diagLower.includes('iterativ')) {
    maternos.push('Iteratividade (cesárea anterior)');
  }

  // Diagnósticos fetais
  if (diagLower.includes('gemelar') || diagLower.includes('gemeos')) {
  ...
  if (diagLower.includes('cardiopatia')) {
    fetais.push('Cardiopatia Fetal');
  }
  if (diagLower.includes('dilatação') || diagLower.includes('dilatacao')) {
    fetais.push('Alteração de Sistema Urinário');
  }

  return { maternos, fetais };
}

function processNotrecareRow(row: NotrecareRow) {
...
export async function importNotrecare2025(rows: NotrecareRow[]): Promise<{
  success: number;
  failed: number;
  errors: string[];
}> {
