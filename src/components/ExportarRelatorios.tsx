...
const ExportarRelatorios = ({ trigger }: ExportarRelatoriosProps) => {
  ...
  const exportarCSV = (data: any[]) => {
    ...
    const rows = data.map(item => [
      ...
      item.status,
      item.data_agendamento_calculada || '',
      item.idade_gestacional_calculada || '',
      item.ig_pretendida,
      Array.isArray(item.procedimentos) ? item.procedimentos.join('; ') : '',
      item.diagnosticos_maternos || '',
      item.diagnosticos_fetais || '',
      item.dum_status,
      item.data_dum || '',
      item.semanas_usg,
      item.dias_usg,
      item.usg_recente,
      item.numero_gestacoes,
      item.numero_partos_cesareas,
      item.numero_partos_normais,
      item.numero_abortos,
      item.historia_obstetrica || '',
      item.indicacao_procedimento,
      item.medicacao || '',
      item.placenta_previa || '',
      item.necessidade_uti_materna || '',
      item.necessidade_reserva_sangue || '',
      item.observacoes_agendamento || '',
      format(new Date(item.created_at), 'dd/MM/yyyy HH:mm', { locale: ptBR }),
      item.aprovado_em ? format(new Date(item.aprovado_em), 'dd/MM/yyyy HH:mm', { locale: ptBR }) : '',
            ...
            <div className="grid grid-cols-2 gap-4">
              <Card 
                className={cn(
                  ...
                  formato === 'csv' && "border-primary bg-primary/5"
                )}
              ...
              <Card 
                className={cn(
                  "cursor-pointer transition-all hover:border-primary opacity-50",
                  formato === 'excel' && "border-primary bg-primary/5"
                )}
                onClick={() => toast.info('Excel nativo em breve')}
              >
                <CardContent className="pt-6 text-center">
            ...
            <div className="space-y-2">
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setIncluirCancelados(checked as boolean)}
                />
                <label
                  htmlFor="cancelados"
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setIncluirRejeitados(checked as boolean)}
                />
                <label
                  htmlFor="rejeitados"
                  className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
