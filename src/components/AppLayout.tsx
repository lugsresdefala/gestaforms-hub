...
const MenuGroup = ({
  ...
  return <SidebarGroup className="mb-2">
      {!collapsed && <button onClick={toggle} className="w-full flex justify-between items-center px-3 py-2 uppercase text-xs font-semibold text-sky-200/70 hover:text-white">
          <span>{title}</span>
          <ChevronDown className={`h-4 w-4 transition-transform ${expanded ? "" : "-rotate-90"}`} />
        </button>}

      {(collapsed || expanded) && <SidebarGroupContent>
          <SidebarMenu>
            {items.map((item: any) => <SidebarMenuItem key={item.title}>
                <SidebarMenuButton asChild>
                  <NavLink to={item.url} end onClick={onItemClick} className="group flex items-center gap-2 rounded-lg px-2 py-2 hover:bg-white/10 hover:backdrop-blur-md transition text-sky-50/80 hover:text-white" activeClassName="bg-orange-500/20 text-white shadow-inner border border-orange-300/40">
                    <item.icon className="h-4 w-4" />
                    {!collapsed && <>
                        <span className="text-sm">{item.title}</span>
                        {item.badge && <Badge className="ml-auto bg-red-500 text-white h-4 px-1.5 text-xs animate-pulse">
                            {item.badge}
                          </Badge>}
                      </>}
                  </NavLink>
  ...
  return <>
      <div className="p-4 sm:p-6 border-b border-sky-400/20 bg-gradient-to-br from-white/5 to-sky-400/10 backdrop-blur-xl">
        <div className="flex flex-col items-center gap-3">
          <div className="p-10 shadow-inner px-[10px] py-[10px] rounded-none opacity-95 border-3 border-popover-foreground bg-accent-foreground border-2">
            <img src="/hapvida-logo.png" alt="Logo" className="h-16 object-cover shadow-inner opacity-95 border-2 rounded-none border-primary-foreground" />
          </div>
      ...
      <div className="py-4 space-y-2">
        <MenuGroup title="Principal" items={sections.principal} groupKey="principal" collapsed={collapsed} onItemClick={onItemClick} />
        <MenuGroup title="Agendamentos" items={sections.agendamentos} groupKey="agendamentos" collapsed={collapsed} onItemClick={onItemClick} />
        <MenuGroup title="Administração" items={sections.administracao} groupKey="admin" collapsed={collapsed} onItemClick={onItemClick} />

        <SidebarGroup>
          {!collapsed && <SidebarGroupLabel className="text-xs px-3 text-sky-200/70">Suporte</SidebarGroupLabel>}
          <SidebarGroupContent>
            <SidebarMenu>
              {sections.suporte.map(item => <SidebarMenuItem key={item.title}>
                  <SidebarMenuButton asChild>
                    <NavLink to={item.url} end onClick={onItemClick} className="group flex items-center gap-2 px-2 py-2 text-sky-50/80 hover:text-white hover:bg-white/10 rounded-lg transition">
                      <item.icon className="h-4 w-4" />
              ...
              <SidebarMenuItem>
                <SidebarMenuButton asChild>
                  <a href="mailto:T_TIAGO.OLIVEIRA@HAPVIDA.COM.BR" className="flex items-center gap-2 px-2 py-2 text-sky-50/80 hover:text-white hover:bg-white/10 rounded-lg transition">
