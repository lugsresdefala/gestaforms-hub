...
const MenuContent = ({ collapsed = false, onItemClick }: { collapsed?: boolean; onItemClick?: () => void }) => {
  ...
  return (
    <>
      {/* Logo Header */}
      <div
        className={`p-4 sm:p-6 border-b border-sky-400/20 bg-gradient-to-br from-white/5 to-sky-400/5 backdrop-blur-xl transition-all duration-300 ${
          collapsed ? "px-3" : ""
        }`}
      >
        {!collapsed ? (
          <div className="flex flex-col gap-3">
            <div className="bg-white/95 backdrop-blur-sm rounded-lg p-3 sm:p-4 shadow-2xl shadow-sky-900/20 border border-sky-200/30">
    ...
    <SidebarProvider>
      <div className="min-h-screen w-full bg-gradient-to-br from-slate-50 via-blue-50/30 to-sky-50/50 flex">
        ...
        <div className="flex-1 flex flex-col min-h-screen w-full lg:w-auto">
          {/* Header - Responsivo */}
          <header
            className={`h-14 sm:h-16 flex items-center justify-between border-b px-3 sm:px-4 lg:px-6 sticky top-0 z-50 transition-all duration-300 ${
              isScrolled
          ...
          >
              ...
              <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
                <SheetTrigger asChild>
                  <Button
                    variant="ghost"
                    size="icon"
                    className="lg:hidden h-9 w-9 hover:bg-orange-50 hover:shadow-md hover:shadow-orange-500/20 transition-all duration-300 rounded-lg"
                  >
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <Button
                ...
                variant="ghost"
                size="icon"
                className="relative h-8 w-8 sm:h-9 sm:w-9 hover:bg-red-50 hover:text-red-600 hover:shadow-md hover:shadow-red-500/20 transition-all duration-300 rounded-lg group"
                title="Sair"
              >
