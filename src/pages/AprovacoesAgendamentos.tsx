...
const AprovacoesAgendamentos = () => {
  ...
  const [datasAprovacao, setDatasAprovacao] = useState<{
    [key: string]: string;
  }>({});
  const [filtroStatus, setFiltroStatus] = useState<string>('pendente');
  const [validacoesProtocolo, setValidacoesProtocolo] = useState<{
    [key: string]: ValidacaoProtocolo;
  }>({});
  useEffect(() => {
    if (!isAdminMed()) {
      toast({
  ...
  const fetchAgendamentosPendentes = async () => {
    setLoading(true);
    let query = supabase.from('agendamentos_obst').select('*').order('created_at', {
      ascending: false
    });
  ...
  const validarProtocoloParaAgendamento = (agendamento: Agendamento, dataAgendamentoStr: string): ValidacaoProtocolo => {
    try {
      // Calcular IG atual
      const dataUsg = new Date(agendamento.data_primeiro_usg);
      const igUsg = calcularIgPorUsg(dataUsg, agendamento.semanas_usg, agendamento.dias_usg, new Date());
      let igDum = null;
      ...
      const {
        igFinal
      } = determinarIgFinal(igDum, igUsg, agendamento.semanas_usg);
  ...
  const handleAprovar = async (agendamentoId: string) => {
    ...
    const {
      error
    } = await supabase.from('agendamentos_obst').update({
      status: 'aprovado',
  ...
  return <div className="container mx-auto p-6 space-y-6">
      <div>
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-3xl font-bold">Aprovações de Agendamentos</h1>
        </div>
      ...
      {agendamentos.length === 0 ? <Card>
          ...
          {agendamentos.map(agendamento => <Card key={agendamento.id} className="hover:shadow-lg transition-shadow">
              <CardHeader>
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <CardTitle className="text-xl flex items-center gap-2 flex-wrap">
                      {agendamento.nome_completo}
                      {agendamento.status === 'aprovado' && <Badge variant="default" className="bg-green-500">
                          <Check className="h-3 w-3 mr-1" />
                          Aprovado
                        </Badge>}
                      {agendamento.status === 'rejeitado' && <Badge variant="destructive">
                          <X className="h-3 w-3 mr-1" />
                          Rejeitado
                        </Badge>}
                      {agendamento.status === 'pendente' && <Badge variant="secondary">
                    ...
                    <CardDescription className="mt-2 space-y-1">
                      ...
                      <p>Solicitado em: {format(new Date(agendamento.created_at), "dd/MM/yyyy 'às' HH:mm", {
                    locale: ptBR
                  })}</p>
                    </CardDescription>
                  </div>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="p-4 border rounded-lg bg-background">
                    ...
                    {agendamento.data_agendamento_calculada ? <Badge className="text-base">
                        {format(new Date(agendamento.data_agendamento_calculada), 'dd/MM/yyyy')}
                      </Badge> : <p className="text-sm text-muted-foreground">Sem data calculada</p>}
                  </div>
                  <div className="p-4 border rounded-lg bg-background">
                    <p className="text-sm font-semibold mb-2">Definir data final (obrigatório)</p>
                    <Input type="date" value={datasAprovacao[agendamento.id] || ''} onChange={e => handleDataChange(agendamento.id, e.target.value)} />
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {agendamento.procedimentos.map((proc, idx) => <Badge key={idx} variant="secondary">{proc}</Badge>)}
                  </div>
                </div>

                {/* Validação de Protocolo */}
                {validacoesProtocolo[agendamento.id] && datasAprovacao[agendamento.id] && <div className="space-y-3 border-t pt-4">
                    ...
                    {!validacoesProtocolo[agendamento.id].compativel && <Alert variant="destructive" className="border-2 border-red-600 bg-red-100 dark:bg-red-950/30">
                    ...
                    {validacoesProtocolo[agendamento.id].compativel && <Alert className="border-green-500 bg-green-50 dark:bg-green-950/20">
                    ...
                    {validacoesProtocolo[agendamento.id].alertas.length > 0 && <Alert className="border-orange-600 bg-orange-50 dark:bg-orange-950/20 border-2">
                        ...
                        <AlertDescription>
                          <ul className="list-disc list-inside space-y-2 text-sm text-orange-700 dark:text-orange-300 mt-2 font-medium">
                            {validacoesProtocolo[agendamento.id].alertas.map((alerta, idx) => <li key={idx} className="leading-relaxed">{alerta}</li>)}
                          </ul>
                        </AlertDescription>
                      </Alert>}

                    {validacoesProtocolo[agendamento.id].recomendacoes.length > 0 && <Alert className="border-blue-500 bg-blue-50 dark:bg-blue-950/20">
                        ...
                        <AlertDescription>
                          <ul className="list-disc list-inside space-y-1 text-sm text-blue-600 dark:text-blue-400 mt-2">
                            {validacoesProtocolo[agendamento.id].recomendacoes.map((rec, idx) => <li key={idx}>{rec}</li>)}
                          </ul>
                        </AlertDescription>
                      </Alert>}
                  </div>}
                ...
                <div className="space-y-2">
                  ...
                  <Textarea value={observacoes[agendamento.id] || ''} onChange={e => setObservacoes({
              ...observacoes,
              [agendamento.id]: e.target.value
            })} placeholder="Adicione observações sobre este agendamento..." rows={3} />
                </div>

                <div className="flex gap-3 pt-4 border-t">
                  {agendamento.status === 'pendente' ? <>
                      <Button onClick={() => handleAprovar(agendamento.id)} className="flex-1 bg-green-600 hover:bg-green-700" size="lg">
                        <Check className="h-5 w-5 mr-2" />
                        Aprovar Agendamento
                      </Button>
                      <Button onClick={() => navigate(`/editar-agendamento/${agendamento.id}`)} variant="outline" size="lg">
                        <Edit className="h-4 w-4 mr-2" />
                        Editar
                      </Button>
                      <Button onClick={() => handleRejeitar(agendamento.id)} variant="destructive" className="flex-1" size="lg">
                        <X className="h-5 w-5 mr-2" />
                        Rejeitar Agendamento
                      </Button>
                    </> : <Button onClick={() => navigate(`/editar-agendamento/${agendamento.id}`)} variant="outline" className="flex-1" size="lg">
                      <Edit className="h-4 w-4 mr-2" />
                      Editar Agendamento
                    </Button>}
                </div>
