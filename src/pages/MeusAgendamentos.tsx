...
const MeusAgendamentos = () => {
  ...
  const {
    refreshKey
  } = useRealtimeAgendamentos(user?.id);
  useEffect(() => {
  ...
  const fetchMeusAgendamentos = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.from('agendamentos_obst').select('*').eq('created_by', user.id).order('created_at', {
        ascending: false
      });
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Solicitado em: {format(new Date(agendamento.created_at), "dd/MM/yyyy 'às' HH:mm", {
                    locale: ptBR
                  })}
                      </p>
                    </div>
                    <div className="flex flex-col gap-2 items-end">
                      ...
                      {new Date(agendamento.data_agendamento_calculada) < new Date() && <Badge variant="secondary" className="bg-green-600 text-white">
                          Resolvido
                        </Badge>}
                    </div>
                  </div>
                ...
                <CardContent className="space-y-3">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {agendamento.procedimentos.map((proc, idx) => <Badge key={idx} variant="outline">{proc}</Badge>)}
                    </div>
                  </div>
                  ...
                  {agendamento.status === 'aprovado' && agendamento.aprovado_em && <div className="bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800 rounded-lg p-3">
                      <p className="text-sm font-medium text-green-800 dark:text-green-200">
                        ✓ Aprovado em {format(new Date(agendamento.aprovado_em), "dd/MM/yyyy 'às' HH:mm", {
                  locale: ptBR
                })}
                      </p>
                      {agendamento.observacoes_aprovacao && <p className="text-sm text-green-700 dark:text-green-300 mt-1">
                          {agendamento.observacoes_aprovacao}
                        </p>}
                    </div>}

                  {agendamento.status === 'rejeitado' && <div className="bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 rounded-lg p-3">
                      <p className="text-sm font-medium text-red-800 dark:text-red-200">
                        ✗ Rejeitado
                      </p>
                      {agendamento.observacoes_aprovacao && <p className="text-sm text-red-700 dark:text-red-300 mt-1">
                          Motivo: {agendamento.observacoes_aprovacao}
                        </p>}
                    </div>}

                  {agendamento.status === 'pendente' && <div className="bg-yellow-50 dark:bg-yellow-950 border border-yellow-200 dark:border-yellow-800 rounded-lg p-3">
